...
//pins
#define DisplayPower 13 //питание дисплея
#define rxPin 3         // выход приемника int0
#define RVR_Vcc_Pin 8   // включение питания приемника
#define txPin 2         // вход передатчика
#define TM_Vcc_Pin 9    // питание передачика
#define TM_gnd_Pin 12    // земля передатчика
#define speakerPin 7   // Спикер, он же buzzer, он же beeper
#define spr_gnd_Pin 10  // Земля спикера
#define G_Led LED_BUILTIN        // всроенный свеодиод
#define Btn_ok_Pin 5    // Кнопка оправки текущего кода
#define Btn_left_Pin 6  // Кнопка влево
#define Btn_right_Pin 4 // Кнопка вправо
...
void setup() {
 ...
 // pinMode(DisplayPower, OUTPUT); digitalWrite(DisplayPower, HIGH); //Включения подачи питания на дисплей
...
void sendSynthBit(int bt[2]){
  ...
  for (byte i=0; i < 2; i++){
    if (bt[i] > 0){
      PORTB |= 1 << 1; 
      digitalWrite(txPin, HIGH); //Pin 9 arduino
      myDelayMcs(bt[i]);
    } else{
      PORTB &= ~(1 << 1); 
      digitalWrite(txPin, LOW);
...
void loop() {
  ...
  if ((millis() - stTimer > (100000 >> prescal)) && (snifferMode == smdNormal)) go2sleep(); //засыпаем через 10 сек
}
